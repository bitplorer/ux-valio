...
on the same hook. Persist the hashed password on ``add_task_post_set`` of the
last field so a failed set does not consume the name. Do not invent
``add_pre_set``. A uniqueness **task** is the wrong bag — uniqueness is a
process (it can reject). Persist is a task (side effect after store).
...
class Registration:
    ...
    @password_confirm.add_task_post_set
