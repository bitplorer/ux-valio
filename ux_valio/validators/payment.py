...
# leftover: previous helper name. Prefer ``_luhn_valid``.
_luhn_correctness = _luhn_valid


def _is_card_of(pattern: re.Pattern[str], card_number: str) -> bool:
    return _luhn_valid(card_number) and pattern.fullmatch(card_number) is not None
