...
## Leftover teaching: Soft 4 TypeError-at-register was temporary
...
it. Soft 4 then refused `async def` at `add_*` (`_require_sync_callable`) so
ux-valio would not store a coroutine object or leak a never-awaited task.
That fail-closed was **honesty**, not a product claim that async is illegal
forever.

Soft 5 restores valio's registration (async callables may hang on `add_*`)
and locks run rules that do **not** bring `asyncio.run` /
`run_until_complete` back inside `Property.__set__`.
...
`add_*_task` accept async defs. No blanket `_require_sync_callable`.
