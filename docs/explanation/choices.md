...
`BytesValidator` / `IntegerEnumValidator`; the extra is compile-once + one FFI
...
IntegerEnum member set (`compile_integer_enum` / `apply_integer_enum`). Pattern
and named identity stay on the host. Next: StringEnum.
