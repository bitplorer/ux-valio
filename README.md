...
pip install ux-valio[native]         # optional Integer/Float/String/Bytes apply peer
```
...
`StringValidator(max_length=50)` / `BytesValidator(min_length=1)` may compile once and `apply` in one FFI
