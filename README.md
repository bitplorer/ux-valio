...
pip install ux-valio[native]         # optional Integer/Float/String/Bytes/IntegerEnum apply peer
```
...
`IntegerEnumValidator()` on a concrete `IntEnum` field may compile once
and `apply` in one FFI when `ux-valio[native]` is installed; stdlib
Python apply is the default without it. Measure with
`python benches/measure_host_peer.py` (plan apply vs setattr — not a
70× product-setattr claim). Cap Door B stays off this path.
