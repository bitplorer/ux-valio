...
pip install ux-valio[native]         # optional Integer/Float bound apply peer
```
...
`IntegerValidator(min_value=0)` / `FloatValidator(min_value=0.0)` may compile once and `apply` in one FFI
