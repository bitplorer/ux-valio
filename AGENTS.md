...
- Optional native apply (PyO3 peer, host decides / peer applies) is
  **not** a second door. Soul of the instance stays Python. See
  ...
  and IntegerEnum member-set apply). Optional extra
  ``ux-valio[native]`` (sibling crate ``native/``, module
  ``ux_valio_native``) binds closed Integer and Float bound plans,
  closed String and Bytes length plans, and a closed IntegerEnum
  member set at
  construct: ``MinValue`` / ``MaxValue`` / ``GreaterThan`` /
  ``LessThan`` / ``Equal`` and min+max range, plus ``MinLength`` /
  ``MaxLength`` / ``Length``, plus ``Member`` values from the concrete
  ``IntEnum``, compile once, one FFI
  ``apply`` / ``apply_float`` / ``apply_string`` / ``apply_bytes`` /
  ``apply_integer_enum`` per set. ``compile_integer_enum`` and
  ``apply_integer_enum`` stay a separate pair. Integer type door is FFI
  ...
  ``isinstance`` of the concrete enum, then FFI ``i64`` extract of the
  member; the set miss uses the host type-door ``TypeError``. Open
  TypeValidator / plain EnumValidator / StringEnum stay on the host.
...
`_closed_string_length`, `_closed_bytes_length`, `_closed_length`, `_closed_integer_enum_members`, `_closed_value_bounds`, `_bind_compiled_plan`, `_apply_native_closed`,
...
`_raise_host_float_type_miss`, `_raise_host_string_type_miss`, `_raise_host_bytes_type_miss`, `_raise_host_integer_enum_type_miss`, `_raise_host_closed_type_miss`,
