...
- Optional native apply (PyO3 peer, host decides / peer applies) is
  **not** a second door. Soul of the instance stays Python. See
  ...
  than native Integer / Float bound apply). Optional extra
  ``ux-valio[native]`` (sibling crate ``native/``, module
  ``ux_valio_native``) binds closed Integer and Float bound plans at
  construct: ``MinValue`` / ``MaxValue`` / ``GreaterThan`` /
  ``LessThan`` / ``Equal`` and min+max range, compile once, one FFI
  ``apply`` / ``apply_float`` per set. Integer type door is FFI
  ``i64`` extract; Float is FFI ``f64`` extract (IEEE Door A: NaN
  unordered on min/max/gt/lt, ``eq`` uses ``!=`` so NaN never
  matches). Open TypeValidator / String length stay on the host.
...
`_closed_value_bounds`, `_bind_compiled_plan`, `_apply_native_closed`,
...
`_raise_host_float_type_miss`, `_raise_host_closed_type_miss`,
