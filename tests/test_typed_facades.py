...
def test_datetime_post_validate_cannot_store_a_plain_date():
    ...
    field.add_process_post_validate(to_date, namespace=When)
    with pytest.raises(TypeError):
...
def test_uuid_post_validate_cannot_store_a_str():
    ...
    field.add_process_post_validate(smash, namespace=Row)
    with pytest.raises(TypeError):
        Row(u=raw)
...
def test_url_post_validate_cannot_store_a_lie():
    ...
    field.add_process_post_validate(smash, namespace=Link)
    with pytest.raises(ValueError, match="URL"):
