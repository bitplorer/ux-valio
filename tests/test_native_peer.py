...
"""Optional ``ux-valio[native]`` peer: Integer/Float bounds, String/Bytes length, IntegerEnum/StringEnum members, Boolean and Decimal type doors.
...
def test_decimal_compiles_once_at_bind():
    ...
    plan = field._native_plan
    assert plan is not None
    assert field._native_apply is peer.apply_decimal
    assert field._native_apply is not peer.apply_float
