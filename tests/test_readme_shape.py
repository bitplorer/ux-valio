...
def test_readme_teaches_facade_primary_and_chain_is_allof():
    ...
    assert "named.us.postal" in text or "us/" in text
    assert "named.uk.postal" in text or "uk/" in text
    ...
    assert "ITINValidator" in text
    assert "USStateValidator" in text
    assert "IndiaStateCodeValidator" in text
