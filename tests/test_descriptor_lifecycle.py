...
def test_post_get_does_not_replace_in_flight_never_set_error():
    ...
    field.add_process_post_get(boom, namespace=Box)
    box = Box.__new__(Box)
    with pytest.raises(AttributeError, match=r"Box\.s is not set") as exc:
        _ = box.s
    assert type(exc.value) is AttributeError
...
def test_post_validate_cannot_store_type_mismatch():
    ...
    field.add_process_post_validate(smash, namespace=N)
...
def test_post_validate_cannot_store_named_facade_lie():
    ...
    email.add_process_post_validate(not_email, namespace=Contact)
    ...
    card.add_process_post_validate(not_card, namespace=Wallet)
    ...
    when.add_process_post_validate(to_datetime, namespace=Day)
