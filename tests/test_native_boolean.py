...
"""Native module: Boolean type door (``Plan::Boolean``, exact ``bool``).
...
def test_boolean_compiles_once_at_bind():
    ...
    assert field._native_apply is native.apply_boolean
    assert field._native_apply is not native.apply_integer
...
def test_native_boolean_uses_apply_boolean_not_integer_apply():
    ...
    plan = native.compile_boolean()
    assert native.apply_boolean(plan, True) is None
    assert native.apply_boolean(plan, False) is None
    with pytest.raises(TypeError):
        native.apply_boolean(plan, 1)
    with pytest.raises(TypeError):
        native.apply_boolean(plan, 0)
...
@needs_native
def test_unexpected_boolean_native_bind_raises_runtime_error(monkeypatch):
...
@needs_native
def test_unexpected_boolean_native_apply_raises_runtime_error():
    field = BooleanValidator(debug=True, name="n")
    assert field._native_plan is not None
