...
"""Processors run then tasks once. Only ``pre_validate`` / ``post_validate`` return is stored.

There is no hang named ``pre_set`` / ``_processors["pre_set"]`` bag. Hang
before-store work on ``pre_validate``. ``post_set`` runs after store and
...
def test_validator_runs_processing_then_task_once():
    ...
    _bind_phase(v, HookHost._owner_key(Host), log, "pre_validate", "task_pre_validate", "pre")
...
def test_validator_all_phases_run_task_once_after_processing():
    ...
    phases = (
        ("pre_validate", "task_pre_validate", "pre"),
        ("post_validate", "task_post_validate", "post"),
        ("post_set", "task_post_set", "set"),
        ("pre_get", "task_pre_get", "pget"),
        ("post_get", "task_post_get", "gget"),
        ("pre_delete", "task_pre_delete", "pdel"),
        ("post_delete", "task_post_delete", "gdel"),
    )
...
def test_plain_validator_processing_without_tasks():
    ...
    v.pre_validate(proc, namespace=Plain)
...
def test_only_pre_set_processor_return_is_stored():
    ...
    v.post_set(post_set_proc, namespace=Host)
